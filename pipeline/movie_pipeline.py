...
from ml.helpers import predict_lgbm, fit_bert, fit_lgbm, predict_bert, ensemble
...
def main():
    ...
    # Initialize transformer for wikipedia data
    wiki_preprocessor = WikiProcessor()

    # Add additional data and clean data
    training_df = concat_trains(spark, "../datasets/imdb/")
    wiki_df = wiki_preprocessor.transform(wiki_df)
    ...
    # Train and inference using BERTs
    bert_outputs_validation = []
    bert_outputs_test = []
    for _ in range(4):
        # Train Bert and LGBM models
        bert_model = fit_bert(training_df, "gpu")
        bert_outputs_validation.append(predict_bert(validation_df, bert_model, "gpu"))
        bert_outputs_test.append(predict_bert(test_df, bert_model, "gpu"))

        # Remove model from GPU and clear memory, so we can re-train new BERT from scratch
        torch.cuda.empty_cache()

    # Train and inference using LGBM
    lgbm_model = fit_lgbm(training_df)
    validation_prediction = predict_lgbm(validation_df, lgbm_model)
    test_prediction = predict_lgbm(test_df, lgbm_model)

    validation_output, hidden_test_output = ensemble(bert_outputs_validation, bert_outputs_test, validation_prediction,
                                                     test_prediction)

    # Save results for submission
    pd.DataFrame(validation_output)[0].map({1: "True", 0: "False"}).\
        to_csv("../datasets/lightGBM_outputs/validation_ensemble_4bert.csv", index=False, header=False)
    pd.DataFrame(hidden_test_output)[0].map({1: "True", 0: "False"}).\
        to_csv("../datasets/lightGBM_outputs/test_ensemble_4bert.csv", index=False, header=False)
